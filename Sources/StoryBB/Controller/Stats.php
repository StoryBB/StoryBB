...
class Stats implements Routable
{
	public static function register_own_routes(RouteCollection $routes): void
	{
		$routes->add('stats', new Route('/stats', ['_function' => [static::class, 'display_stats']]));
	}
	...
	public static function display_stats()
	{
		...
		if (isset($_REQUEST['xml']))
		{
			...
			static::getDailyStats('YEAR(date) = {int:year} AND MONTH(date) = {int:month}', ['year' => $year, 'month' => $month]);
		...
		// Build the link tree......
		$context['linktree'][] = [
			'url' => $urlgenerator->generate('stats'),
		...
		while ($row_months = $smcFunc['db']->fetch_assoc($months_result))
		{
			...
			$context['yearly'][$row_months['stats_year']]['months'][(int) $row_months['stats_month']] = [
				...
				'href' => $urlgenerator->generate('stats', [($expanded ? 'collapse' : 'expand') => $ID_MONTH, '_fragment' => 'm' . $ID_MONTH]),
				'link' => '<a href="' . $urlgenerator->generate('stats', [($expanded ? 'collapse' : 'expand') => $ID_MONTH, '_fragment' => 'm' . $ID_MONTH]) . '">' . $txt['months'][(int) $row_months['stats_month']] . ' ' . $row_months['stats_year'] . '</a>',
		...
		static::getDailyStats(implode(' OR ', $condition_text), $condition_params);
	 ...
	 */
	public static function getDailyStats($condition_string, $condition_parameters = [])
	{
