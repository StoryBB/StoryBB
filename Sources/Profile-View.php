 ...
 */
function fetch_alerts($memID, $all = false, $counter = 0, $pagination = [], $withSender = true)
{
...
function showAlerts($memID)
{
	...
	// Get the alerts.
	$context['alerts'] = fetch_alerts($memID, true, false, ['start' => $start, 'maxIndex' => $maxIndex]);
	$toMark = false;
	$action = '';
...
function tracking($memID)
{
	...
	$subActions = [
		'activity' => ['trackActivity', $txt['trackActivity'], 'moderate_forum'],
		'ip' => ['TrackIP', $txt['trackIP'], 'moderate_forum'],
		'edits' => ['trackEdits', $txt['trackEdits'], 'moderate_forum'],
		'groupreq' => ['trackGroupReq', $txt['trackGroupRequests'], 'approve_group_requests'],
		'logins' => ['TrackLogins', $txt['trackLogins'], 'moderate_forum'],
	];
 ...
 */
function list_getUserErrorCount($where, $where_vars = [])
{
	global $smcFunc;
 ...
 */
function list_getUserErrors($start, $items_per_page, $sort, $where, $where_vars = [])
{
 ...
 * @return int count of messages matching the IP
 */
function list_getLoginCount($where, $where_vars = [])
{
	global $smcFunc;
 ...
 */
function list_getLogins($start, $items_per_page, $sort, $where, $where_vars = [])
{
	global $smcFunc;
