 ...
 * @see Subs-Db-mysql.php#sbb_db_initiate
 *
...
function sbb_db_query($identifier, $db_string, $db_values = array(), $connection = null)
{
	...
	if ($db_last_result === false && empty($db_values['db_error_skip']))
		$db_last_result = sbb_db_error($db_string, $connection);
 ...
 *
 */
function sbb_db_affected_rows($result = null)
{
 ...
 */
function sbb_db_fetch_row($request, $counter = false)
{
	global $db_row_count;
 ...
 */
function sbb_db_fetch_assoc($request, $counter = false)
{
	global $db_row_count;
 ...
 */
function sbb_db_data_seek($request, $counter)
{
	global $db_row_count;
 ...
 */
function sbb_db_unescape_string($string)
{
 ...
 */
function sbb_db_select_db($db_name, $db_connection)
{
	return true;
}
...
function sbb_db_error_insert($error_array)
{
	...
	if (empty($pg_error_data_prep))
			$pg_error_data_prep = pg_prepare($db_connection, 'sbb_log_errors',
				'INSERT INTO ' . $db_prefix . 'log_errors(id_member, log_time, ip, url, message, session, error_type, file, line)
				VALUES( $1, $2, $3, $4, $5, $6, $7, $8, $9)'
			);

	pg_execute($db_connection, 'sbb_log_errors', $error_array);
}
