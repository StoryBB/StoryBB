...
function ViewModlog()
{
		...
		'additional_rows' => [
			[
				...
				'value' => '
					...
					<input type="submit" name="removeall" value="' . $txt['modlog_removeall'] . '" data-confirm="' . $txt['modlog_remove_all_confirm'] . '" class="you_sure">' : ''),
				'class' => 'floatright',
			],
		],
	];
	...
	call_integration_hook('integrate_viewModLog', [&$listOptions, &$moderation_menu_name]);
...
function list_getModLogEntryCount($query_string = '', $query_params = [], $log_type = 1, $ignore_boards = false)
{
	...
	list ($entry_count) = $smcFunc['db']->fetch_row($result);
...
function list_getModLogEntries($start, $items_per_page, $sort, $query_string = '', $query_params = [], $log_type = 1, $ignore_boards = false)
{
	...
	while ($row = $smcFunc['db']->fetch_assoc($result))
	{
		...
		$entries[$row['id_action']] = [
			'id' => $row['id_action'],
			'ip' => $seeIP ? IP::format($row['ip']) : $txt['logged'],
