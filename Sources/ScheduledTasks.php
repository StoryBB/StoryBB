...
function scheduled_fetchSMfiles()
{
	...
	$file_data = fetch_web_data("http://www.storybb.org/updates.json");
	$updates = json_decode($file_data);
	...
	$smcFunc['db_query']('substring', '
		...
		WHERE id_file = 1',
		array(
			'file_data' => $updates->current_version,
		)
	);
	...
	$smcFunc['db_query']('substring', '
		...
		WHERE id_file = 2',
		array(
			'file_data' => json_encode($updates->sbbAnnouncements),
		)
	);
