...
function ManagePaidSubscriptions()
{
	...
	if (!empty($modSettings['paid_enabled']))
		$subActions = [
			'modify' => ['ModifySubscription', 'admin_forum'],
			'modifyuser' => ['ModifyUserSubscription', 'admin_forum'],
			'settings' => ['ModifySubscriptionSettings', 'admin_forum'],
			'view' => ['ViewSubscriptions', 'admin_forum'],
			'viewsub' => ['ViewSubscribedUsers', 'admin_forum'],
		];
	else
		$subActions = [
			'settings' => ['ModifySubscriptionSettings', 'admin_forum'],
	...
	if (!empty($modSettings['paid_enabled']))
		$context[$context['admin_menu_name']]['tab_data']['tabs'] = [
			'view' => [
	...
	routing_integration_hook('integrate_manage_subscriptions', [&$subActions]);
...
function ModifySubscriptionSettings($return_config = false)
{
	...
	if (!empty($modSettings['paid_enabled']))
	{
		...
		if (!empty($modSettings['paid_currency_code']) && !in_array($modSettings['paid_currency_code'], ['usd', 'eur', 'gbp', 'cad', 'aud']))
		...
		$config_vars = [
			...
			'',
				...
				['email', 'paid_email_to', 'subtext' => $txt['paid_email_to_desc'], 'size' => 60],
			'',
				'dummy_currency' => ['select', 'paid_currency', ['usd' => $txt['usd'], 'eur' => $txt['eur'], 'gbp' => $txt['gbp'], 'cad' => $txt['cad'], 'aud' => $txt['aud'], 'other' => $txt['other']], 'javascript' => 'onchange="toggleOther();"'],
		...
		foreach ($gateways as $gateway)
		{
			...
			{
				$config_vars[] = ['title', $gatewayClass->getShortTitle(), 'text_label' => $gatewayClass->getDisplayTitle()];
...
function ModifySubscription()
{
	...
	if (isset($_POST['delete_confirm']) && isset($_REQUEST['delete']))
	{
		...
		if (!empty($members))
		{
			...
			if ($smcFunc['db']->num_rows($request))
				list ($id_group, $add_groups) = $smcFunc['db']->fetch_row($request);
			$smcFunc['db']->free_result($request);
	...
	if (isset($_POST['save']))
	{
		...
		else
		{
			...
			list ($disableGroups) = $smcFunc['db']->fetch_row($request);
			$smcFunc['db']->free_result($request);
		...
		call_integration_hook('integrate_save_subscription', [($context['action_type'] == 'add' ? $id_subscribe : $context['sub_id']), $_POST['name'], $_POST['desc'], $isActive, $span, $cost, $_POST['prim_group'], $addgroups, $isRepeatable, $allowpartial, $emailComplete, $reminder]);
	...
	else
	{
		...
		list ($context['disable_groups']) = $smcFunc['db']->fetch_row($request);
		$smcFunc['db']->free_result($request);
...
function ViewSubscribedUsers()
{
	...
	$search_vars = empty($_POST['sub_search']) ? [] : ['search' => '%' . $_POST['sub_search'] . '%', 'guest' => $txt['guest']];
...
function list_getSubscribedUserCount($id_sub, $search_string, $search_vars = [])
{
	...
	list ($memberCount) = $smcFunc['db']->fetch_row($request);
	$smcFunc['db']->free_result($request);
...
function ModifyUserSubscription()
{
	...
	if ($context['log_id'] && !$context['sub_id'])
	{
		...
		if ($smcFunc['db']->num_rows($request) == 0)
			fatal_lang_error('no_access', false);
		list ($context['sub_id']) = $smcFunc['db']->fetch_row($request);
		$smcFunc['db']->free_result($request);
	}
	...
	elseif (isset($_REQUEST['save_sub']))
	{
		...
		if (empty($context['log_id']))
		{
			...
			list ($id_member, $id_group) = $smcFunc['db']->fetch_row($request);
			$smcFunc['db']->free_result($request);
		...
		else
		{
			...
			list ($id_member, $old_status) = $smcFunc['db']->fetch_row($request);
			$smcFunc['db']->free_result($request);
	...
	if ($context['action_type'] == 'add')
	{
		...
		if (isset($_GET['uid']))
		{
			...
			list ($context['sub']['username']) = $smcFunc['db']->fetch_row($request);
			$smcFunc['db']->free_result($request);
		}
		else
...
function addSubscription($id_subscribe, $id_member, $renewal = 0, $forceStartTime = 0, $forceEndTime = 0)
{
	...
	if ($smcFunc['db']->num_rows($request) != 0)
	{
		list ($id_sublog, $endtime, $starttime) = $smcFunc['db']->fetch_row($request);
	...
	list ($old_id_group, $additional_groups) = $smcFunc['db']->fetch_row($request);
	$smcFunc['db']->free_result($request);
...
function removeSubscription($id_subscribe, $id_member, $delete = false)
{
	...
	list ($id_group, $additional_groups) = $smcFunc['db']->fetch_row($request);
	$smcFunc['db']->free_result($request);
