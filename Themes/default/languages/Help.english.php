...
$helptxt['non_immersive_mode'] = 'When immersive mode is not enabled, different behaviours may be relevant; in a roleplay that focuses on a single setting or universe, the roleplay game is happening throughout the forum and there is no huge need to restrict which character posts where. This is provided for with simple mode.<br><br>In a multiple setting forum, however, where multiple different games are running at any one time, being able to quickly switch between only the right characters that can post in a given area - this is contextual mode.';
