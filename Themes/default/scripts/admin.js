...
function testFTP()
{
	...
	// Post the data out.
	sendXMLDocument(smf_prepareScriptUrl(sbb_scripturl) + 'action=admin;area=packages;sa=ftptest;xml;' + sbb_session_var + '=' + sbb_session_id, sPostData, testFTPResults);
}
...
function expandFolder(folderIdent, folderReal)
{
	...
	{
		ajax_indicator(true);
		getXMLDocument(smf_prepareScriptUrl(sbb_scripturl) + 'action=admin;area=packages;onlyfind=' + escape(folderReal) + ';sa=perms;xml;' + sbb_session_var + '=' + sbb_session_id, onNewFolderReceived);
	}
	// Otherwise reload.
	else
		return true;
